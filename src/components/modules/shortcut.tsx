...
export default function Shortcut({name,iconPath,url}:Props){
    ...
    return(
        ...
        <div className="opacity-0  hover:bg-[var(--tint)] hover:shadow-lg group-hover:opacity-100 duration-500 z-2 absolute right-2 top-2 flex items-center justify-center w-6 h-6 p-[4px] rounded-full" onClick={()=>{setDropdownVisible(!dropdownVisible)}}>
           ...
           <div className="relative w-auto">
            ...
            options={["Edit","Remove"]}
            toggleIsVisible={()=>{setDropdownVisible(!dropdownVisible)}}
            />
           </div>
        </div>
        ...
        <p className="text-center" onClick={redirect}>
        {name}</p>
        </div>
    );
